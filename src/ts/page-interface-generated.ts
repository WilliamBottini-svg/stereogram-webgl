/* tslint:disable */
/* Generated by `npm run build-page` when Demopage.build succeeds; inlined for environments where page generation fails. */
...
declare namespace Page.Helpers {
    ...
    export class Cache<T extends ICacheable> {
        public constructor(objectsName: string, loadObjectsFunction: LoadObjectsFunction<T>);
    ...
    export class Storage<T extends IStorable> {
        public constructor(
            prefix: string,
            serialize: (control: T) => string | null,
            tryDeserialize: (controlId: string, serializedValue: string) => boolean,
        );
...
declare namespace Page.Tabs {
    ...
    function clearStoredState(tabsId: string): void;
}
...
declare namespace Page.FileControl {
    function addUploadObserver(id: string, observer: (filesList: FileList) => unknown): void;
    function addDownloadObserver(id: string, observer: () => unknown): void;
